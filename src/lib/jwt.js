...
export async function setAuthCookies(admin){
  const accessToken = await signToken({sub: admin.id, role: admin.role}, ACCESS_SECRET, "15m");
  const refreshToken = await signToken({sub: admin.id, role: admin.role}, REFRESH_SECRET, "7d");
