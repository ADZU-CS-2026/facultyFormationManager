...
export default function CSITEPage() {
  const csiteData = rawData["CSITE"];

  // Header information
  const office = csiteData?.[0]?.["Ateneo de Zamboanga University"];
  const profileTitle = csiteData?.[1]?.["Ateneo de Zamboanga University"];
  const schoolYear = csiteData?.[2]?.["Ateneo de Zamboanga University"];
  const department =
    csiteData?.find(
      (row) =>
  ...
  const tableHeaderRow = csiteData?.find((row) => row?.["Column2"] === "Last Name ");
  const tableRows = csiteData?.filter(
...
<<<<<<< HEAD
              <div style={{fontSize: "18px"}}>College of Science and Information Technology Records</div>
            </div>
...
=======
              <div>College of Science and Information Technology Records</div>
            </div>
